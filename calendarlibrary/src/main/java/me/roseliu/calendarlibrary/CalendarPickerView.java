...
public class CalendarPickerView extends ListView {
  ...
  final List<Calendar> selectedCals = new ArrayList<>(); //TODO
  final List<Calendar> highlightedCals = new ArrayList<>(); //TODO
  ...
  public boolean selectDate(Date date, boolean smoothScroll) {//TODO
    validateDate(date);
...
//            selectedCell.setRangeState(RangeState.NONE); }
    ...
    if (date != null) {
      ...
      if(selectionMode == SelectionMode.MULTIPLE_RANGE && selectedCells.size() > 1){

        //if start and end in borrow dates list: then set Range date to normal, otherwise to lend states
        Date start = selectedCells.get(0).getDate();
        Date end = selectedCells.get(1).getDate();
        selectedCells.get(0).setRangeState(RangeState.FIRST_LEND);
        selectedCells.get(1).setRangeState(RangeState.LAST_LEND);

        int startMonthIndex = cells.getIndexOfKey(monthKey(selectedCals.get(selectedCals.size()-2)));
        int endMonthIndex = cells.getIndexOfKey(monthKey(selectedCals.get(selectedCals.size()-1)));
            ...
            for (MonthCellDescriptor singleCell : week) {
              if (singleCell.getDate().after(start)
                ...
                if(highlightedCells.contains(singleCell)){
                  ...
                  singleCell.setRangeState(RangeState.MIDDLE_LEND);
                  selectedCells.add(singleCell);
                }
  ...
  public void highlightDates(Collection<Date> dates) { //TODO
    for (Date date : dates) {
      validateDate(date);
  ...
  public void deactivateDates(ArrayList<Integer> deactivatedDates){ //TODO
